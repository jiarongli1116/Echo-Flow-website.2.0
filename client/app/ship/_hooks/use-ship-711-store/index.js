...
import useInterval from '@/hooks/use-interval-711';
import useLocalStorage from '@/hooks/use-localstorage-711';
...
export function useShip711StoreOpener(
  ...
  // 如果使用localStorage，才會使用localStorage的值作為預設值
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    subscribe('set-store', (e) => {
      console.log('收到 711 門市選擇資料:', e.detail); // 添加調試日誌
...
export function useShip711StoreCallback(keyLocalStorage = 'store711') {
  ...
  useEffect(() => {
    ...
    console.log('711 回調頁面收到的參數:', params); // 添加調試日誌
    ...
    if (window.opener) {
      ...
      console.log('已發送 711 門市資料到母視窗:', params); // 添加調試日誌
    } else {
      console.error('無法找到母視窗 (window.opener)'); // 添加錯誤日誌
    }

    // 設定到localStorage
    setValue(params);
    console.log('已儲存到 localStorage (key:', keyLocalStorage, '):', params); // 添加調試日誌
