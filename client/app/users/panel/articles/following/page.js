...
import { useUserFollowing } from "@/hooks/use-forums";
// 分頁樣式（沿用 articles/page.module.css，與 coupons 相同）
import styles from "../page.module.css";

const PAGE_SIZE = 12; // ← 一頁 12 張（手機 1 欄 / md 以上 2 欄）
...
export default function FollowingPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.replace("/auth/login?next=/users/panel/articles/following");
      return;
    }
  ...
  const { users, loading, pagination } = useUserFollowing(me?.id || 0, {
    page,
    limit: PAGE_SIZE,
    autoLoad: !!me?.id,
  });
  ...
  const isLoading = Boolean(loading?.users);
  const list = Array.isArray(users) ? users : [];

  if (checking) {
    return (
      <UserPanelLayout pageTitle="我的追蹤">
        ...
        <div className="row">
          <div className="col-12">
            <div className="d-flex justify-content-between align-items-center mb-4">
              ...
              <small className="text-muted">共 {total} 人</small>
            </div>

            {list.length === 0 ? (
              <div className="text-center py-5">
                <i className="bi bi-person-heart display-1 text-muted"></i>
                <p className="mt-3 text-muted">你尚未追蹤任何人。</p>
              </div>
            ) : (
              <>
                {/* 手機 1 欄，md 以上 2 欄 */}
                  ...
                  {list.map((u) => (
                    <div className="col" key={u.id}>
                      <FollowUserCard
                        user={u}
                        onChat={(user) =>
                          router.push(`/forums/info?id=${user.id}`)
                        }
                      />
                    </div>
                  ))}
