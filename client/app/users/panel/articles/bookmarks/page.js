...
import { useUserBookmarkedPosts } from "@/hooks/use-forums";
// 分頁樣式（與 coupons 一致，沿用同層的 page.module.css）
...
export default function BookmarksPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.replace("/auth/login?next=/users/panel/articles/bookmarks");
      return;
    }
  ...
  const { posts, loading, pagination } = useUserBookmarkedPosts(me?.id || 0, {
    page,
    limit: PAGE_SIZE,
    autoLoad: !!me?.id,
  });
  ...
  if (checking) {
    return (
      <UserPanelLayout pageTitle="我的收藏">
          ...
          <div className="col-12">
            ...
            {list.length === 0 ? (
              <div className="text-center py-5">
                ...
                <p className="mt-3 text-muted">你還沒有收藏任何文章。</p>
              </div>
            ) : (
